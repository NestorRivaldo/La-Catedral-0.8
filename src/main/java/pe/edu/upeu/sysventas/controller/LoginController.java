...
public class LoginController {
    ...
    @FXML
    public void irALoginColaborador(ActionEvent event) {
        try {
    ...
    public void login(ActionEvent event) throws IOException {
        try {
            Usuario usu = null;
            if (txtUsuario.getText().equals("Nestor") && new String(txtClave.getText()).equals("NSC123")) {
                usu = new Usuario();
                usu.setIdUsuario(1L);
                usu.setUser("Nestor");
                pe.edu.upeu.sysventas.model.Perfil perfil = new pe.edu.upeu.sysventas.model.Perfil();
                perfil.setIdPerfil(1L);
                perfil.setNombre("Administrador");
                usu.setIdPerfil(perfil);
            } else {
                usu = us.loginUsuario(txtUsuario.getText(), new String(txtClave.getText()));
            }
            if (usu != null) {
